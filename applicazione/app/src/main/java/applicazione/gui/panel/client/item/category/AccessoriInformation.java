...
public class AccessoriInformation extends JScrollPane{
    private static final String ITEM_QUERY = "SELECT * FROM ACESSORI WHERE id_articolo = ?";

    private record Item(String tematica, String id) {
    }
    ...
    public AccessoriInformation(final String itemId) {
        setViewportView(tablePanel);
        try (
                var stm = DAOUtils.prepare(connection, ITEM_QUERY, itemId);
                var rS = stm.executeQuery();) {
            while (rS.next()) {
                this.items.add(new Item(rS.getString("tematica"), rS.getString("id_articolo")));
            }
        } catch (Exception e) {
            throw new DAOException(e);
        }

        String[] column = { "tematica", "id" };
        Object[][] data = new Object[items.size()][column.length];

        for (int i = 0; i < this.items.size(); i++) {
            data[i][0] = this.items.get(i).tematica;
